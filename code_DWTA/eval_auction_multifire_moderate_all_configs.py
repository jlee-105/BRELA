"""Pure multi-fire auction (must_fire=None) on the MODERATE temporal-dilemma
curriculum, across all 12 tiered-benchmark scales -- compares against
eval_auction_moderate_all_configs.py's (1:1 eviction) numbers to check
whether allowing many-to-one target assignment actually helps.
"""
import time
...
@torch.no_grad()
def eval_instance_auction_multifire(V, P, TW, nw, nt, mt, amm, prep, cost):
    ...
    for _ in range(mt):
        ...
        action = auction_round_action_multifire(remaining_value, prob, legal_mask)
...
def run_config(M, N, T):
    ...
    for i in range(N_EVAL):
        ...
        objs.append(eval_instance_auction_multifire(V, np.asarray(P), TW, M, N, T, AMM, PREP, COST))
...
if __name__ == "__main__":
    print(f"{'config':<18}{'auction_multifire':>18}")
