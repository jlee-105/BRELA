"""
Train EdgeAwareGNN_ACTOR_COMM_SINKHORN (common/DWTA_GNN_comm_sinkhorn.py --
comm-layer actor plus a differentiable partial-Sinkhorn coordination bonus)
+ auction refinement, on the MODERATE temporal-dilemma curriculum,
multi-scale (M,N,T ~ U[5,7]). Identical training/eval harness to
rl/DWTA_GNN_TRAIN_comm_multiscale.py (CommSCoPE) -- only the actor class
differs -- so the two are directly comparable: this tests whether the
Sinkhorn bonus closes the remaining gap to Sequential that the comm layer
alone did not (CommSCoPE 0.1465 vs Sequential 0.1432, see
brerla_comm_layer_and_multifire_auction memory).

New file -- does not modify rl/Dynamic_Sampling_GNN_moderate.py,
rl/DWTA_GNN_TRAIN_comm_multiscale.py, or any existing training
script/checkpoint.

Usage: python rl/DWTA_GNN_TRAIN_comm_sinkhorn_multiscale.py --total_steps 400 --eval_every 20
"""
import argparse
import os
import sys
...
def main():
    ...
    parser.add_argument('--tag', type=str, default='', help='appended to output filenames to avoid clobbering (see brerla_comm_layer_and_multifire_auction memory: same-seed reruns overwrote a better checkpoint before)')
    args = parser.parse_args()

    torch.manual_seed(args.seed)

    actor = create_gnn_actor_comm_sinkhorn()
    actor.sinkhorn_iters = args.sinkhorn_iters
    ...
    print(f"[COMM-SINKHORN-MULTISCALE] training on M,N,T ~ U[5,7] each, evaluating zero-shot "
    ...
    best_path = f"result/CommSinkhornSCoPE_multiscale_seed{args.seed}{suffix}_best_actor.pt"
    final_path = f"result/CommSinkhornSCoPE_multiscale_seed{args.seed}{suffix}_final_actor.pt"
    ...
    for step in range(1, args.total_steps + 1):
        ...
        if step % args.eval_every == 0 or step == args.total_steps:
            ...
            print(f"    mean_CommSinkhornSCoPE_across_12_configs={mean_scope:.4f}  "
    ...
    print(f"Saved best actor (step {best_step}, mean_CommSinkhornSCoPE={best_scope_score:.4f}) to {best_path}")
