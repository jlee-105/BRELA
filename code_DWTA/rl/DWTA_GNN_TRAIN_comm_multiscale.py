"""
Train EdgeAwareGNN_ACTOR_COMM (common/DWTA_GNN_comm.py -- Parallel actor with
an added weapon-to-weapon self-attention "communication" layer, see that
file's docstring) + auction refinement, on the MODERATE temporal-dilemma
curriculum, multi-scale (M,N,T ~ U[5,7]). Identical training/eval harness to
rl/DWTA_GNN_TRAIN_moderate_multiscale.py (SCoPE) -- only the actor class
differs -- so the two are directly comparable: this tests whether adding
explicit intra-round weapon coordination closes the gap to Sequential (which
won 11/12 configs against SCoPE, see brerla_moderate_full_baseline_table
memory).

New file -- does not modify rl/Dynamic_Sampling_GNN_moderate.py,
common/DWTA_GNN.py, or any existing training script/checkpoint.

Usage: python rl/DWTA_GNN_TRAIN_comm_multiscale.py --total_steps 1500
"""
import argparse
import os
import sys
...
def main():
    ...
    print(f"[COMM-MULTISCALE] training on M,N,T ~ U[5,7] each, evaluating zero-shot "
    ...
    best_path = f"result/CommSCoPE_multiscale_seed{args.seed}_best_actor.pt"
    final_path = f"result/CommSCoPE_multiscale_seed{args.seed}_final_actor.pt"
    ...
    for step in range(1, args.total_steps + 1):
        ...
        if step % 20 == 0:
            print(f"[COMM-MULTISCALE] step {step}/{args.total_steps} "
        ...
        if step % args.eval_every == 0 or step == args.total_steps:
            print(f"--- [COMM-MULTISCALE] zero-shot eval at step {step} ---", flush=True)
            results = eval_all_configs(actor)
            scope_scores = []
            for M, N, T, raw_score, scope_score in results:
                scope_scores.append(scope_score)
                print(f"    {M}M_{N}N_{T}T: Parallel(raw)={raw_score:.4f}  CommSCoPE(auction)={scope_score:.4f}", flush=True)
            ...
            print(f"    mean_CommSCoPE_across_12_configs={mean_scope:.4f}  "
    ...
    print(f"Saved best actor (step {best_step}, mean_CommSCoPE={best_scope_score:.4f}) to {best_path}")
