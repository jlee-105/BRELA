"""
Train the Sequential actor (common/DWTA_GNN_sequential.py) on the MODERATE
temporal-dilemma curriculum, multi-scale (M,N,T ~ U[5,7]) -- the Sequential
counterpart to rl/DWTA_GNN_TRAIN_moderate_multiscale.py (Parallel/SCoPE).
Same rigor: cosine LR decay, grad-norm clipping (built into
self_play_gnn_sequential_moderate), explicit best-checkpoint selection via
periodic zero-shot eval on all 12 held-out tiered-benchmark configs.

Sequential has no auction-refinement counterpart (that's specific to the
Parallel decoder's per-round simultaneous multi-weapon decisions -- Sequential
already resolves weapon-target coordination one edge at a time within its
own single joint W*T+1 action space), so only ONE score is tracked/reported
per config, not a raw/auction-refined pair.

New file -- does not modify rl/Dynamic_Sampling_GNN_sequential.py or any
existing training script/checkpoint.

Usage: python rl/DWTA_GNN_TRAIN_sequential_moderate_multiscale.py --total_steps 1500
"""
import argparse
import os
import sys
...
from Dynamic_Sampling_GNN_sequential_moderate import self_play_gnn_sequential_moderate
...
@torch.no_grad()
def eval_instance_sequential_score(actor, V, P, TW, nw, nt, mt, amm, prep, cost):
    ...
    for _ in range(mt):
        for _ in range(nw):
            ...
            if (mask > 0).any():
                ...
                action = torch.full((1, 1), nw * nt, dtype=torch.long, device=env.assignment_encoding.device)
...
def eval_all_configs(actor):
    ...
    for M, N, T in ALL_EVAL_CONFIGS:
        ...
        for i in range(N_EVAL):
            ...
            objs.append(eval_instance_sequential_score(actor, V, np.asarray(P), TW, M, N, T, AMM, PREP, COST))
...
def main():
    ...
    print(f"[SEQ-MULTISCALE] training on M,N,T ~ U[5,7] each, evaluating zero-shot "
          f"on {len(ALL_EVAL_CONFIGS)} held-out configs (moderate curriculum) every {args.eval_every} steps")

    best_score = float('inf')
    best_step = -1
    os.makedirs("result", exist_ok=True)
    best_path = f"result/Sequential_multiscale_seed{args.seed}_best_actor.pt"
    final_path = f"result/Sequential_multiscale_seed{args.seed}_final_actor.pt"

    t0 = time.time()
    for step in range(1, args.total_steps + 1):
        actor_loss, epoch_info = self_play_gnn_sequential_moderate(actor, episode=1, epoch=step)
        scheduler.step()

        if step % 20 == 0:
            print(f"[SEQ-MULTISCALE] step {step}/{args.total_steps} "
        ...
        if step % args.eval_every == 0 or step == args.total_steps:
            print(f"--- [SEQ-MULTISCALE] zero-shot eval at step {step} ---", flush=True)
            results = eval_all_configs(actor)
            scores = []
            for M, N, T, score in results:
                scores.append(score)
                print(f"    {M}M_{N}N_{T}T: Sequential={score:.4f}", flush=True)
            ...
            print(f"    mean_Sequential_across_12_configs={mean_score:.4f}  "
    ...
    print(f"Saved best actor (step {best_step}, mean_Sequential={best_score:.4f}) to {best_path}")
