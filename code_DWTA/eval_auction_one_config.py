"""Evaluate the auction-refined Parallel checkpoint on ONE config, fresh
subprocess per config (mirrors eval_parallel_one_config.py)."""
import json
import sys
import time
...
for i in range(min(N_EVAL, len(df))):
    ...
    objs.append(eval_instance_auction(V, P, TW, nw, nt, mt, amm, prep, cost))
