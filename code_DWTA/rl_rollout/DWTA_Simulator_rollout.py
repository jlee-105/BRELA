...
class Environment:
    ...
    def __init__(self, assignment_encoding, weapon_to_target_prob, max_time):
        ...
        self.total_cost = torch.zeros(assignment_encoding.size(0), assignment_encoding.size(1), device=DEVICE)
        # max realistic cost = sum(cost[w] * ammo[w])
        ammo_tensor = self.amm_availability[0, 0, :].float()
    ...
    def update_internal_variables(self, selected_action):
        ...
        if (selected_action < NUM_WEAPONS * NUM_TARGETS).any():
            ...
            assignment_encoding_without_no_action[batch_id, par_id, weapon_index, :, AMMUNITION_FEATURE_INDEX] = remaining_ammunition[batch_id, par_id, weapon_index, :].float()

            # weapon availability update
            assignment_encoding_without_no_action[batch_id, par_id, weapon_index, :, WEAPON_AVAILABILITY_FEATURE_INDEX] = 0.0

            # number of fired
            n_target_hit = self.n_target_hit.clone()[:, :, None, :].expand(batch_size, para_size, NUM_WEAPONS, NUM_TARGETS)/(NUM_TARGETS*MAX_TIME)
            assignment_encoding_without_no_action[batch_id, par_id, :, target_index, TARGET_HIT_COUNT_FEATURE_INDEX] = n_target_hit[batch_id, par_id, :, target_index]

            # target value
            assignment_encoding_without_no_action[:, :, :, :, TARGET_VALUE_INDEX] = self.current_target_value.clone().reshape(batch_size, para_size, NUM_WEAPONS, NUM_TARGETS)/MAX_TARGET_VALUE
            # reshape
    ...
    def time_update(self):
        ...
        assignment_encoding_without_no_action[available_weapon_index[0], available_weapon_index[1], available_weapon_index[2], :, WEAPON_AVAILABILITY_FEATURE_INDEX] = 1.0
        ...
        assignment_encoding_without_no_action[:, :, :, :, WEAPON_WAIT_TIME_FEATURE_INDEX] = updated_wait_time

        # time left
        assignment_encoding_without_no_action[:, :, :, :, TIME_LEFT_FEATURE_INDEX] = self.time_left/MAX_TIME
        self.assignment_encoding[:, :, :-1, :] = assignment_encoding_without_no_action.reshape(batch_size, para_size, NUM_WEAPONS * NUM_TARGETS, -1)
    ...
    def reward_calculation(self, alpha=None):
        if alpha is None:
            alpha = COST_WEIGHT
        current_sum = self.current_target_value[:, :, 0:NUM_TARGETS].sum()
        original_sum = self.original_target_value[:, :, 0:NUM_TARGETS].sum()
        destruction_ratio = 1 - current_sum / (original_sum + 1e-8)
