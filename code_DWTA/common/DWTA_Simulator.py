...
class Environment:
    ...
    def _initialize_cost_tracking(self):
        """Initialize weapon firing cost tracking."""
        cost_list = WEAPON_COST[:self.actual_num_weapons]
        if len(cost_list) < self.actual_num_weapons:
            pad_value = WEAPON_COST[-1] if len(WEAPON_COST) > 0 else 50
            cost_list = cost_list + [pad_value] * (self.actual_num_weapons - len(cost_list))
        self.weapon_cost_per_fire = torch.tensor(cost_list, device=DEVICE, dtype=torch.float32)
        self.total_cost = torch.zeros(self.batch_size, self.para_size, device=DEVICE)
        # max realistic cost = sum(cost[w] * ammo[w]) — all ammo spent
        ammo_tensor = self.ammunition_availability[0, 0, :].float()
        self.max_possible_cost = float((self.weapon_cost_per_fire * ammo_tensor).sum().item())
    ...
    def update_internal_variables(self, selected_action):
        ...
        for batch_id in range(batch_size):
            for par_id in range(para_size):
                ...
                self._batch_update_cost(batch_id, par_id, weapon_index)
    ...
    def update_internal_variables_parallel(self, selected_actions):
        """
        Simultaneous per-weapon update for the parallel multi-pointer decoder.

        Unlike `update_internal_variables`, which applies ONE (weapon,target) edge
        per call (requiring the caller to invoke it once per weapon per time step),
        this applies EVERY weapon's decision for the current time step in a single
        call. Multiple weapons may legally select the same target in the same round
        (DWTA has no hard exclusivity, unlike e.g. VRP) -- the per-target damage
        update `V *= (1 - p)` is applied once per selecting weapon, and since
        multiplication is commutative, the accumulated result is identical
        regardless of the (arbitrary) order weapons are processed in below.

        Args:
            selected_actions: [batch, para, num_weapons] int tensor, each entry in
                {0, ..., num_targets-1} for a target choice, or num_targets for no-op.
        """
        batch_size = selected_actions.size(0)
        para_size = selected_actions.size(1)
        num_weapons = selected_actions.size(2)

        for batch_id in range(batch_size):
            for par_id in range(para_size):
                for weapon_index in range(num_weapons):
                    target_index = selected_actions[batch_id, par_id, weapon_index].item()

                    # Skip no-op (index == num_targets means "this weapon holds fire")
                    if target_index == self.actual_num_targets:
                        continue
        ...
        # Update availability and actions (rebuilds mask + mask_per_weapon for next round)
        self._batch_update_availability_and_actions()
    ...
    def _batch_update_cost(self, batch_id, par_id, weapon_index):
        """Accumulate firing cost for the selected weapon."""
        self.total_cost = self.total_cost.clone()
    ...
    def _batch_update_action_mask(self):
        ...
        # Per-weapon mask for the parallel multi-pointer decoder: [batch, para, W, T+1].
        # Reuses the same legality tensor `available_actions` (pre-flatten) so this stays
        # consistent with the legacy flat `self.mask` by construction. No-op (last column)
        # is always legal per weapon -- it is the fallback whenever weapon m has no
        # available target this round (reloading, out of ammo, or no target in Q_m ready).
        self.mask_per_weapon = torch.cat([
            available_actions,
            torch.ones(self.batch_size, self.para_size, self.actual_num_weapons, 1,
                       device=DEVICE, dtype=available_actions.dtype)
        ], dim=-1)

    def reward_calculation(self, alpha=None):
        """
        Calculate cost-aware reward.
        
        Args:
            alpha: Cost weight override (None uses global COST_WEIGHT)
        Returns:
            reward: Multi-objective reward (higher is better)
        ...
        destruction_ratio = 1 - current_sum / original_sum

        normalized_cost = self.total_cost.sum() / max(self.max_possible_cost, 1e-8)
        reward = alpha * destruction_ratio - (1 - alpha) * normalized_cost
