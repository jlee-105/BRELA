...
\usepackage{bm}
\usepackage{url}
% svjour3 already defines proposition/lemma/corollary/remark environments; do not \newtheorem them again.
...
many-to-one auction that assigns targets to the firing weapons by greedy
marginal value, with a provable per-stage approximation guarantee;
a weapon-to-weapon self-attention (communication) layer that lets each
weapon's fire-or-hold decision, not just its target choice, condition on a
learned summary of every other weapon's state that stage; and a learned
repair policy that revisits the completed schedule, flips individual
fire-or-hold decisions, re-simulates, and keeps the best result, which by
construction cannot degrade the schedule it is given and reduces the mean
objective by a further 13\% relative across all tested configurations. Trained only on
...
A separate line instead learns to \emph{improve a complete solution}, repeatedly selecting a local modification and accepting it if it helps: neural local rewriting~\cite{chen2019rewriter}, learning-based improvement heuristics for routing~\cite{wu2021improvement}, and dual-aspect representations for the same~\cite{ma2021dact}. Closest in shape to our pipeline, Zhang, Hu, and Zhao~\cite{zhang2025vrptw} construct a preliminary solution for the vehicle routing problem with time windows using an attention-based policy and then improve it with a large neighborhood search; the difference is the improvement operator, which in their case is a classical, hand-designed destroy-and-repair search, whereas ours is a second learned policy whose edits are accepted by re-simulation.
%% Verified 2026-08-23 against the Springer abstract (doi:10.1007/s10489-025-06829-z, Applied Intelligence 55(13):937):
%% DRL attention policy generates preliminary solutions, classical LNS iteratively improves them -- the
%% construct-then-improve characterization above is confirmed; the LNS is not described as learned.
...
%% weapon node = features [ammunition, availability, remaining time, waiting time] = {a_t,u_t,r_t,d_t}. Correct as written.
...
We call the architecture without this layer SCoPE and with it SCoPE-Comm, and report both. Comparing the two isolates the value of coordination over the participation decision, since coordination over target choice is already handled by the auction (Section~\ref{sec:coordination}). The released implementation additionally carries a learned scalar that can mix a Sinkhorn-normalized bonus into the edge scores; it is initialized at zero, trains to $0.021$, and forcing it to zero at inference changes the mean objective by less than $10^{-4}$ across the twelve held-out configurations, so the term is inactive and plays no role in the results reported here.
...
The repair policy is trained by REINFORCE over the edit sequence, with the reward for each edit being the objective improvement that edit produced (Section~\ref{sec:training}). The edit operator is the simplest one that can express both directions of the participation decision. We also implemented a move operator that reschedules a weapon's shot from one stage to another, and a combined operator that can choose either type of edit at each step. Under an identical frozen-construction protocol in earlier experimentation, the three operators finished within a fraction of a percentage point of one another, with no operator clearly dominating; we kept the single flip for its simplicity and for the smaller action space it gives the jointly trained repair policy. That comparison predates the many-to-one pipeline and has not been re-run under it (Section~\ref{sec:repair-results}).
%% CORRECTED 2026-08-23: the previous sentence here claimed move/combined were "worse than the single flip
%% under an identical protocol" -- the recovered logs contradict that. Frozen CommSinkhorn base, 1:1 auction,
%% n_edits=3 train / 30 eval, identical for all three: flip best 0.1327 (improve_train_log.txt), move best
%% 0.1330 (move_train_log.txt), combined best 0.1312 (combined_train_log.txt), base 0.1357. A near-tie with
%% combined marginally ahead, not a flip win. These 1:1-era numbers are deliberately not reported as tables
%% (incomparable with the many-to-one pipeline, Lemma~\ref{lem:dispersion}).
...
\textbf{Repair policy.} The repair policy is trained with the same estimator on its own episode, the sequence of edits to one schedule (two edits per training step). Its reward per edit is the objective change that edit produced under re-simulation. Credit assignment is easier than for construction, since each edit's reward measures that edit alone rather than a return shared across the whole episode. Edits are sampled during training and chosen by argmax at inference. The two policies are trained jointly on the same instance stream, both updated at every training step, so the repair policy learns on the schedules the construction policy actually produces rather than on those of a frozen proposer; the repair policy uses its own learning rate (Table~\ref{tab:hyperparameters}).
%% Verified 2026-08-23 against the recovered launch command for the reported "mfloop" checkpoints:
%% python rl/DWTA_GNN_TRAIN_joint_multiscale.py --total_steps 400 --eval_every 20 --n_edits 2 --eval_edits 10
%%   --batch_size 15 --para_size 10 --base_ckpt none --mode simultaneous --curriculum moderate --auction multifire --tag mfloop --seed 5
%% i.e. mode=simultaneous (both agents every step), improve_lr default 1e-4, proposer trained from scratch.
...
\centering
\small
\caption{Test problem configurations. Each configuration uses ten fixed, seed-controlled held-out instances, identical across all evaluated methods.}
\label{tab:problem_instance_scales}
\begin{tabular}{c|c|c}
\hline
\textbf{Tier} & \textbf{(M,N,T)} & \textbf{Instances} \\ \hline
...
Optimizer & Adam \\
Learning rate (construction) & $5 \times 10^{-4}$ (cosine decay to $1\%$) \\
Learning rate (repair) & $1 \times 10^{-4}$ (cosine decay to $1\%$) \\
Weight decay & $1 \times 10^{-2}$ \\
Gradient-norm clip & 1.0 \\
Instances per training step & 15 \\
Parallel rollouts per instance $P$ & 10 \\
Repair edit budget $K$ at evaluation & 3 (sweep in Table~\ref{tab:repair-sweep}) \\ \hline
\end{tabular}
\end{table}
...
Table~\ref{tab:efficiency} reports mean per-instance decode time on the same held-out benchmark (ten instances per configuration, two untimed warm-up instances, single GPU). The SCoPE-Comm column times the full construction pipeline of Algorithm~\ref{alg:scope}, including the many-to-one auction at every stage; the Sequential column times the per-weapon decoding loop. SCoPE-Comm is 3.2 times faster at Small tier, and the advantage widens with scale to 5.6 times at Battlefield tier, consistent in direction with the $O(T)$ vs.\ $O(M{\times}T)$ decoding-call complexity argument of Section~\ref{sec:parallel-decoder}. The measured ratio is smaller than the call-count ratio $M$ because the auction runs on the CPU inside the loop and its cost also grows with the number of firing weapons. At the largest configuration, $70{\times}100{\times}15$, the absolute difference is 4.84 versus 0.85 seconds per instance, which is the difference between one decision cycle and several at a battlefield re-planning tempo. The repair layer is not included in these times; each edit costs one additional re-simulation of the construction pass, so a budget of $K$ multiplies the pipeline cost by at most $K{+}1$, and the anytime property of Proposition~\ref{prop:repair-monotone} lets the operator choose $K$ to fit the available cycle.

\begin{table}[htbp]
\centering
\small
\caption{Mean per-instance decode time in seconds (tier averages over three configurations each, ten instances per configuration, single seed). SCoPE-Comm includes the many-to-one auction at every stage; repair is excluded (each edit adds one re-simulation).}
\label{tab:efficiency}
\begin{tabular}{l|ccc}
\hline
\textbf{Tier} & \textbf{Sequential} & \textbf{SCoPE-Comm} & \textbf{Speedup} \\ \hline
Small & 0.080 & 0.025 & 3.2$\times$ \\
Medium & 0.229 & 0.047 & 4.9$\times$ \\
Large & 1.148 & 0.222 & 5.2$\times$ \\
Battlefield & 3.467 & 0.621 & 5.6$\times$ \\ \hline
\end{tabular}
\end{table}

%% Measured 2026-08-23 via eval_inference_timing_moderate.py (result/inference_timing_multifire.txt),
%% Sequential_multiscale_seed5_best vs CommSCoPE_multiscale_seed5_best + auction_round_action_multifire.
%% Per-config range: 2.7x (5,5,5) to 5.7x (70,100,15); mean 5.4x. The pre-revision 4-25x numbers used the
%% auction-free decoder and are superseded.
...
\textbf{Data Availability:} The test instances, trained model checkpoints, and evaluation code are available at \url{https://github.com/jlee-105/BRELA}.
