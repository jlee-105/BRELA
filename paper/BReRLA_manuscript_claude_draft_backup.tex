...
SCoPE computes a solution to Eqs.~\eqref{eq:objective_function}--\eqref{eq:initial_ammunition} in two passes.

\textbf{Pass 1 (construction).} For $t = 1, \ldots, T$: (i) encode $s_t$ as a heterogeneous graph and compute embeddings by $L$ rounds of message passing (Section~\ref{sec:encoding}); (ii) apply one self-attention layer over the $M$ weapon embeddings (Section~\ref{sec:comm}); (iii) evaluate the $M$ per-weapon policies $\pi_1, \ldots, \pi_M$ in one forward pass and extract the participation decisions $\text{fire}_m = \mathbb{1}[\arg\max \pi_m \neq \text{noop}]$ (Section~\ref{sec:parallel-decoder}); (iv) assign targets to the firing weapons by a capacitated auction (Section~\ref{sec:auction}); (v) apply the joint action in the environment. The per-stage cost is one network call, versus $M$ for a sequential decoder.

\textbf{Pass 2 (repair).} Given the completed schedule, a second policy selects a stage-weapon slot $(t,m)$, flips its participation decision, re-simulates the episode, and keeps the best schedule found; this repeats for $K$ edits (Section~\ref{sec:repair}).

The division of labor: target assignment given the firing set is solvable within the round with an approximation guarantee (Proposition~\ref{prop:auction-half}), so it is not learned. The two decisions that depend on future rounds -- fire-or-hold now, and revising a past stage -- are learned. Both policies are trained by reward-to-go REINFORCE with a shared parallel-rollout baseline and no critic (Section~\ref{sec:training}). Algorithm~\ref{alg:scope} states the procedure; Fig.~\ref{fig:overall} illustrates it.
...
\For{$t = 1$ \textbf{to} $T$}
  ...
  \State $a_t \gets \textsc{Auction}(\{m : \text{fire}_m\}, s_t)$ \Comment{capacitated, Sec.~\ref{sec:auction}}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\columnwidth]{figure/Overall.png}
  \caption{The SCoPE pipeline, corresponding step for step to Algorithm~\ref{alg:scope}. Construction repeats the encode/communicate/commit/auction cycle once per stage, the environment transition closing the loop; the assembled schedule then passes to the repair layer, which scores every stage-weapon slot, flips one, re-simulates, and keeps the best result over $K$ edits. Shading distinguishes learned components from fixed ones.}
...
The modeling choice here is the graph, not the network that runs on it. Message passing itself is standard~\cite{gilmer2017neural}, so we describe the representation carefully and the machinery briefly.

Eq.~\eqref{eq:state} writes the state as one $\mathbb{R}^9$ vector per (weapon,target) pair, which overstates how much of it is genuinely pairwise. Only the damage rate $p = P_{m,n}$ varies with both indices. Four of the nine entries belong to weapon $m$ alone -- readiness $u_t$, remaining reload $d_t$, remaining ammunition $a_t$, and the stage counter $r_t$
...
-- and four to target $n$ alone: remaining value $v_t$, cumulative hits $f_t$, and the window bounds $ws, we$. Flattening all nine into $M \times N$ independent vectors therefore replicates each weapon's own state $N$ times and each target's $M$ times, and asks the network to rediscover from data that those copies are the same quantity.

We instead give the state the shape it already has: weapon nodes $\bm{x}_m^{\text{wpn}} \in \mathbb{R}^4$, target nodes $\bm{x}_n^{\text{tgt}} \in \mathbb{R}^4$, and edges $\bm{x}_{m,n}^{\text{edge}} \in \mathbb{R}^1$ carrying $P_{m,n}$, present only for $n \in Q_m$ so that engagement compatibility is a property of the graph's topology rather than a mask applied afterwards. Each is linearly projected to a common width $H$, and $L$ rounds of message passing follow: an edge is refreshed from its two endpoints and its own previous value, and each node is then updated by a GRU from the sum of its incident edges. Because $N$ and $M$ enter only through those sums, the same parameters apply at any instance size -- the property the zero-shot scale generalization of Section~\ref{sec:data} rests on.

Two consequences of this encoding are used later and are worth naming now. First, after $L$ rounds every weapon embedding $\bm{h}_m^{\text{wpn},(L)}$ depends on every target's remaining value and every other weapon's readiness. Second, the edge embedding $\bm{h}_{m,n}^{\text{edge},(L)}$ has absorbed target $n$'s own node embedding, so any score computed from that edge is already conditioned on how much of target $n$ is left. Together these give the weapons a common, accurate view of the objects they are coordinating over -- which, as Section~\ref{sec:parallel-decoder} argues, carries them a long way without any explicit conflict resolution, and, as Section~\ref{sec:comm} shows, not all the way.

\subsection{Weapon-to-Weapon Communication}
\label{sec:comm}

The graph encoding gives every weapon the same view of the battlefield, but no view of the other weapons' current decisions. Weapons in similar situations therefore make similar decisions: if the shared state favors firing, many weapons fire, whether or not the stage needs that many shots. The auction of Section~\ref{sec:auction} cannot correct this, because it only reassigns targets among weapons that have already decided to fire. Under resource abundance the error is minor. Under scarcity, how many weapons should fire in a given stage is the central decision, and nothing in the architecture so far addresses it.
...
\end{equation}
where $\text{MHA}$ is standard multi-head self-attention~\cite{vaswani2017attention} over the weapon axis. Unlike the mean-pooled context $\bar{\bm{h}}^{\text{wpn}}$, attention lets each weapon weight the specific weapons relevant to its own decision, such as those competing for the same compatible targets or those still holding ammunition. Both scoring heads of Section~\ref{sec:parallel-decoder} consume $\bm{h}_m^{\text{wpn,comm}}$ (Eqs.~\eqref{eq:edge-score}--\eqref{eq:noop-score}). The added cost is one attention pass over $M$ tokens per stage, independent of $N$, so the $O(T)$ decoding-call complexity is unchanged.

We call the architecture without this layer SCoPE and with it SCoPE-Comm, and report both. Comparing them isolates the value of participation-level coordination, since target-level coordination is already handled by the auction (Section~\ref{sec:coordination}).

\subsection{Parallel Multi-Pointer Policy}
\label{sec:parallel-decoder}

Unlike a single global policy over all $M \times N$ (weapon,target) pairs (as in earlier neural WTAP/DWTAP formulations, e.g.~\cite{na2023weapon}), SCoPE decodes $M$ \emph{independent} distributions simultaneously, one per weapon, each over its own compatible targets plus a no-op. Both scoring heads read the communicated weapon embedding $\bm{h}_m^{\text{wpn,comm}}$ of Eq.~\eqref{eq:comm-out}:
\begin{equation}
\label{eq:edge-score}
...
\end{equation}
where $\text{Res}(\bm{x}) = \bm{x} + \bm{W}_2^{\text{res}}\,\sigma(\bm{W}_1^{\text{res}}\,\text{LN}(\bm{x}))$ is a residual block with layer normalization, $\bm{W}_c$ and $\bm{W}_0$ project their concatenated inputs to $\mathbb{R}^H$, and $\bar{\bm{h}}^{\text{wpn}} = \frac{1}{M}\sum_m \bm{h}_m^{\text{wpn,comm}}$, $\bar{\bm{h}}^{\text{tgt}} = \frac{1}{N}\sum_n \bm{h}_n^{\text{tgt},(L)}$ are pooled global-context vectors. The per-weapon policy is
\begin{equation}
\label{eq:parallel-policy}
\pi_m(\cdot \mid s_t) = \text{softmax}\!\left(\left[\text{score}_{m,n}\right]_{n \in Q_m},\ \text{score}_{m,\text{noop}}\right)
\end{equation}
masked to targets currently available under the constraints of Section~\ref{sec:problem} (reload, ammunition, time window); no-op is always legal, and a weapon that is reloading or out of ammunition is deterministically assigned no-op and contributes no gradient term that round. All $M$ policies $\pi_1, \ldots, \pi_M$ are computed in a single forward pass and sampled independently, giving the joint action $a_t = (a_{1,t}, \ldots, a_{M,t})$ with
\begin{equation}
\log \pi_\theta(a_t \mid s_t) = \sum_{m=1}^{M} \log \pi_m(a_{m,t} \mid s_t).
\end{equation}
Because the $M$ policies are sampled independently, several weapons may select the same target in the same stage. This is a feasible action under the constraints of Section~\ref{sec:problem}, so we do not prevent it with a conflict handler as in hard-coupled problems~\cite{parco2024}. However, independent sampling introduces two kinds of coordination error. First, multiple weapons may choose the same target when spreading their fire would have destroyed more value. Second, the wrong number of weapons may choose to fire at all. The first error concerns target assignment and is corrected by the auction in Section~\ref{sec:auction}. The second error concerns participation. The auction cannot correct it, because the auction only reassigns targets among weapons that have already decided to fire. Section~\ref{sec:comm} addresses this participation error, and Section~\ref{sec:coordination} measures both errors empirically.

\subsection{Auction Refinement of Target Assignment}
\label{sec:auction}

The policy of Eq.~\eqref{eq:parallel-policy} makes two decisions in one softmax: whether weapon $m$ fires, and which target it engages. At inference we separate them. The fire-or-hold decision
\begin{equation}
\text{fire}_m := \mathbb{1}\!\left[\arg\max_n \pi_m(n \mid s_t) \neq \text{noop}\right]
\end{equation}
is kept from the policy. The target assignment is then recomputed by a non-learned auction over the firing set $F = \{m : \text{fire}_m = 1\}$.

The reason for this separation is that the two decisions have different structure. Given $F$, the target assignment problem is fully specified within the stage: it maximizes the stage objective $f_t$ of Eq.~\eqref{eq:round-obj} subject to Eq.~\eqref{eq:weapon_target_constraint}, and all of its data ($V_n\rho_n^{(t)}$, $P_{m,n}$, the legality mask) is known when the stage begins. Proposition~\ref{prop:auction-half} shows the auction below solves this problem with a $\tfrac12$-approximation guarantee. The participation decision has no such formulation: a weapon that holds fire contributes zero to $f_t$, and the value of holding is realized only in later stages. Target assignment is therefore computed exactly by a fixed mechanism, and only the participation decision is learned. This is the reverse of~\cite{sun2026grabbing}, where reinforcement learning is used to learn the auction's bidding values; the overall pattern of pairing a learned decision with a classical combinatorial method follows~\cite{delarue2020rl}.

Our first implementation used Bertsekas's auction algorithm for the one-to-one assignment problem~\cite{bertsekas1988auction}. Each firing weapon bids $V_n\rho_n^{(t)} P_{m,n} - \text{price}(n)$ for its legal targets $n \in Q_m$ and takes the best one; an incumbent on that target is evicted and re-bids. This algorithm enforces at most one weapon per target (Lemma~\ref{lem:dispersion}). DWTAP does not: several weapons may legally engage one target in the same stage, and the optimal allocation sometimes requires it (Corollary~\ref{cor:excluded-optima}). The measured consequence appears in Section~\ref{sec:coordination}: per-stage dispersion under the eviction auction is exactly $1.000$ on every instance, against $0.88$--$0.92$ for the sequential decoder.

We therefore replaced it with a capacitated variant, following the transportation-problem generalization of Bertsekas and Casta\~{n}\'{o}n~\cite{bertsekas1989transportation}. Let $S_n \subseteq F$ be the weapons already assigned to target $n$ this stage and
\begin{equation}
\label{eq:survival-factor}
\sigma_n = \prod_{m' \in S_n} (1 - P_{m',n})
\end{equation}
its accumulated survival factor. The marginal value of assigning weapon $m$ to target $n$ is
\begin{equation}
\label{eq:marginal-bid}
b_{m,n} = V_n\,\rho_n^{(t)}\,\sigma_n\,P_{m,n}.
\end{equation}
The auction repeatedly selects the pair $(m,n)$ with the largest $b_{m,n}$ over all unassigned $m \in F$ and legal $n$, assigns it, and updates $\sigma_n$; assignments are never undone. Diminishing returns is built into the bid through $\sigma_n$, so no per-target capacity parameter is needed: an earlier version capped assignments at $K$ weapons per target, but the uncapped version is as good or better at every scale tested, with wasteful redundancy (fire landing on targets already near zero survival) remaining at zero (Section~\ref{sec:coordination}).

\subsection{Learned Repair of the Assembled Schedule}
\label{sec:repair}

Every mechanism to this point decides within a stage and then moves on. The auction reallocates targets among the weapons firing at stage $t$; the communication layer lets those weapons' participation decisions condition on one another at stage $t$. Neither can revisit stage $t$ once stage $t{+}1$ has begun, and some of what makes a stage-$t$ decision good or bad is only observable afterwards -- whether the ammunition it spent turned out to be needed later, whether the target it left alive was still there when its window closed. A construction policy, parallel or sequential, is committed to that judgment at the moment it decides.

We therefore add a second learned policy that operates on the \emph{completed} schedule. Let $\phi \in \{0,1\}^{T\times M}$ be a flip mask over stage-weapon slots, and let the repaired participation decision at slot $(t,m)$ be $\text{fire}_m^{(t)} \oplus \phi_{t,m}$: where $\phi_{t,m}=1$, a weapon that fired instead holds, and a weapon that held instead fires. The episode is then re-simulated end to end under the modified decisions, with the auction of Section~\ref{sec:auction} reassigning targets exactly as before. The repair policy scores all $T\times M$ slots from the states recorded along the current trajectory, flips the highest-scoring unflipped slot, re-simulates, and repeats for a budget of $K$ edits, retaining the best schedule encountered.

Two properties follow from that construction rather than from training, and both matter for a real-time setting.
...
\end{proposition}
\begin{proof}
The returned schedule is the best of those evaluated, and the unedited schedule is always among them, giving $J_K \leq J_0$. Edits are applied greedily and never undone, so the sequence of schedules evaluated under budget $K$ is a prefix of that evaluated under $K' \geq K$; the minimum over a superset cannot increase.
\end{proof}

The first half of Proposition~\ref{prop:repair-monotone} is what makes the layer safe to deploy: a repair policy that has learned nothing useful, or is evaluated on instances unlike those it trained on, cannot degrade the schedule it was given -- the worst case is that its edits are discarded and the construction policy's own output is returned. The second half makes $K$ an anytime control. Repair costs one re-simulation per edit and requires no retraining to change $K$, so the same trained pair can be run at $K$ small when the decision cycle is tight and $K$ large when it is not, trading wall-clock for quality along a curve rather than choosing an architecture in advance.

Feasibility needs no special handling, which is worth stating because it is the usual difficulty with local search over schedules. A flip changes only the fire-or-hold \emph{intent} at one slot; the assignment that intent produces is still computed by the auction against the environment's own legality mask (readiness, ammunition, time window, and $Q_m$), and the episode is re-simulated from the start rather than patched in place. Downstream consequences -- ammunition freed for a later stage, a reload clock that now begins two stages later -- are recomputed exactly, and an edit that asks a weapon to fire when it has no legal target is simply absorbed, leaving the schedule unchanged. The only cost of such an edit is that it consumes one unit of the budget, which is why candidate slots are restricted to those that currently fire (where a flip cancels a real engagement) or are currently legal (where a flip can add one).

Training is by REINFORCE over the edit sequence, with the reward for each edit being the objective improvement that edit produced. This is a materially easier learning problem than the construction policy's: the credit for an edit is the measured change it caused, attributed to that edit alone, rather than a return shared across every decision in an episode. We sample edits during training and take the argmax at inference, and use the same shared-baseline normalization across parallel rollouts as in Section~\ref{sec:training}.

The edit operator is deliberately the simplest one that can express both directions of the participation decision. We also implemented a \emph{move} operator, which atomically reschedules a weapon's shot from one stage to another, on the reasoning that ``hold now so this weapon is free two stages later'' requires two coordinated flips and a greedy selector would never take the first alone. Measured against the single-flip operator under an identical protocol, move-only was worse, and a combined operator that could choose either at each step -- whose neighbourhood strictly contains the single-flip neighbourhood -- was also worse, the larger two-stage action space evidently costing more in learning difficulty than the added reach returned. We report single-flip as the operator used throughout and the other two as measured negative results (Section~\ref{sec:repair-results}).

We denote the full pipeline -- SCoPE-Comm followed by the repair layer -- as SCoPE-Repair.

\subsection{Theoretical Analysis: What Simultaneous Decoding Can and Cannot Guarantee}
\label{sec:submodular}

The preceding argument for why simultaneous decoding is viable is qualitative. This section makes the underlying trade-off precise. The honest form of the result is asymmetric: the sequential information structure inherits a constant-factor guarantee, while the zero-communication parallel structure provably does not -- its worst case degrades linearly in $M$. The value of stating both is that it locates the burden of explanation correctly: the small gaps observed in Section~\ref{sec:coordination} are a property of DWTAP's soft coupling and of shared-state conditioning, not of a universal worst-case guarantee.

\textbf{Submodularity of the per-round objective.} Fix round $t$ and let $\rho_n^{(t)}\in[0,1]$ denote target $n$'s surviving value fraction entering round $t$, so $V_n\rho_n^{(t)}$ is its remaining value under Eq.~\eqref{eq:objective_function}. Writing $S_t \subseteq E_t := \{(m,n): n\in Q_m,\ (m,n)\ \text{feasible at } t\}$ for the pairs assigned in round $t$, the value destroyed that round is
\begin{equation}
\label{eq:round-obj}
f_t(S_t) = \sum_{n=1}^N V_n\rho_n^{(t)}\left(1 - \!\!\prod_{m:(m,n)\in S_t}\!\!(1-P_{m,n})\right).
\end{equation}
Each summand is a weighted coverage-type function, and a nonnegative sum of monotone submodular functions is monotone submodular, so $f_t$ is monotone submodular on $E_t$. The round-$t$ feasibility constraint (Eq.~\eqref{eq:weapon_target_constraint}, at most one target per weapon) restricts $S_t$ to a \emph{partition matroid} over $E_t$, partitioned by weapon. Let $\text{OPT}_t = \max_{S_t\ \text{feasible}} f_t(S_t)$.

\begin{proposition}[Sequential decoding: constant-factor guarantee]
\label{prop:seq-half}
An idealized sequential decision rule -- each weapon, in turn, selecting the feasible pair of largest marginal gain given all pairs already selected this round -- is exactly the greedy algorithm for monotone submodular maximization under a matroid constraint, and therefore guarantees $f_t(\mathrm{Seq}_t) \geq \tfrac12\,\text{OPT}_t$~\cite{fisher1978analysis}.
\end{proposition}

\begin{proposition}[Parallel decoding without communication: tight $\Theta(1/M)$]
\label{prop:par-linear}
Consider the idealized zero-communication parallel rule: each weapon $m$ simultaneously selects its own feasible pair of largest \emph{singleton} value $f_t(\{(m,n)\})$, with no visibility into the other weapons' same-round choices. Then
\begin{equation}
...
The same machinery settles what the auction of Section~\ref{sec:auction} is worth, and this is the result that licenses confining learning to the participation decision. Once the firing set is fixed, the auction is not an approximation of unclear quality -- it is matroid greedy on $f_t$, exactly.

\begin{proposition}[The capacitated auction is matroid greedy on the round objective]
\label{prop:auction-half}
Fix round $t$ and a set $F \subseteq \{1,\ldots,M\}$ of firing weapons, and let $\text{OPT}_t(F)$ be the maximum of $f_t$ over feasible allocations that assign each weapon of $F$ at most one target. The capacitated auction of Section~\ref{sec:auction} returns an allocation $S^{\text{auc}}_t$ with
\begin{equation}
f_t(S^{\text{auc}}_t) \;\geq\; \tfrac12\,\text{OPT}_t(F).
\end{equation}
\end{proposition}
\begin{proof}
First, the auction's bid is the exact marginal gain. Let $S$ be the allocation so far and $S_n \subseteq F$ the weapons already assigned to target $n$ in $S$. Adding $(m',n')$ changes only the $n'$ summand of Eq.~\eqref{eq:round-obj}:
\begin{align*}
f_t(S \cup \{(m',n')\}) - f_t(S)
...
&= V_{n'}\rho_{n'}^{(t)}\,\sigma_{n'}\,P_{m',n'},
\end{align*}
with $\sigma_{n'} = \prod_{m \in S_{n'}}(1-P_{m,n'})$ the survival factor accrued at $n'$ this round. This is verbatim the quantity the auction ranks by (Section~\ref{sec:auction}).

Second, the auction is greedy in that quantity. Each iteration scans all still-unassigned weapons of $F$ against all their legal targets, takes the single largest bid, records the assignment, updates $\sigma$, and never revokes an assignment. By the identity above it therefore adds, at every step, the feasible element of largest marginal gain given all elements chosen so far.

Feasible allocations -- at most one target per weapon, weapons drawn from $F$ -- form a partition matroid over $E_t$ restricted to $F$, and $f_t$ is monotone submodular on that ground set. Greedy under a matroid constraint on a monotone submodular objective attains at least $\tfrac12$ of the constrained optimum~\cite{fisher1978analysis}, which is the claim.
\end{proof}

Proposition~\ref{prop:auction-half} justifies the division of labor in Section~\ref{sec:auction}: the within-round assignment already carries a scale-independent constant-factor guarantee with no learned parameters, while no analogous statement is possible for the participation decision, since $f_t$ assigns zero to a held weapon and the value of holding lies outside the round.

Propositions~\ref{prop:seq-half} and~\ref{prop:par-linear} bound what the two \emph{decoding} structures can achieve. The refinement mechanism that sits between them carries a constraint of its own, and because that constraint is a property of the mechanism rather than of the problem, it is worth stating separately.

\begin{lemma}[The eviction auction enforces one-to-one assignment]
\label{lem:dispersion}
In any round, the eviction-based auction of Section~\ref{sec:auction} assigns at most one weapon to each target.
\end{lemma}
\begin{proof}
The auction maintains a map from targets to their currently assigned weapon. When weapon $m$ claims target $n$, any incumbent holding $n$ is evicted and returned to the unassigned queue before $m$ is recorded as $n$'s owner, so the map remains single-valued after every bid. It is single-valued initially, and the procedure terminates with it single-valued; hence no target ends the round with two assigned weapons.
\end{proof}

\begin{corollary}[Excluded optima]
\label{cor:excluded-optima}
If the round-optimal allocation $S_t^\star$ assigns two or more weapons to some target, the eviction auction cannot produce it, and such instances exist.
\end{corollary}
\begin{proof}
The first clause is immediate from Lemma~\ref{lem:dispersion}. For existence, take two weapons and two targets with $P \equiv \tfrac12$, remaining values $V_1\rho_1 = 1$ and $V_2\rho_2 = \varepsilon$, and all pairs feasible. Concentrating both weapons on target 1 destroys $1 - (\tfrac12)^2 = \tfrac34$; spreading them destroys $\tfrac12 + \tfrac{\varepsilon}{2}$, which is smaller for $\varepsilon < \tfrac12$. The optimum therefore double-teams target 1, and Lemma~\ref{lem:dispersion} forbids it.
\end{proof}

The practical significance of Corollary~\ref{cor:excluded-optima} is that the loss is invisible in the objective values the mechanism reports: the excluded allocations are never proposed, so nothing in the auction's own output indicates that a better assignment was unreachable. Diagnosing it requires measuring the assignment's \emph{structure} rather than its value, which is why Section~\ref{sec:coordination} reports per-round dispersion -- the ratio of distinct engaged targets to firing weapons -- alongside objective. Lemma~\ref{lem:dispersion} predicts a dispersion of exactly $1.000$ under eviction, at every round of every instance, and that is what we measure; a sequential decoder on the same instances concentrates fire enough to run at $0.88$--$0.92$ while wasting almost none of it. The capacitated auction of Section~\ref{sec:auction} removes the constraint, and does so without the pile-on that Proposition~\ref{prop:par-linear}'s construction warns about, because a target's marginal value there is already discounted by the survival factor accrued from earlier assignments in the same round.

\begin{remark}
\label{rem:gap-interpretation}
Propositions~\ref{prop:seq-half} and~\ref{prop:par-linear} concern idealized, information-structure-matched decision rules, not our trained networks, and they bound single-round allocations rather than full $T$-round trajectories. Their point is the separation: sequential decoding carries a scale-independent $\tfrac12$ guarantee, while parallel decoding without within-round communication can lose a factor linear in $M$ in the worst case. Against that backdrop, the measured destruction-ratio gap of under 2 percentage points at every tier (Section~\ref{sec:coordination}) is informative rather than trivial. Three structural properties of DWTAP explain it. First, the adversarial construction in Proposition~\ref{prop:par-linear} requires many weapons' top choices to coincide on one target while near-equal alternatives go untouched; SCoPE's score functions are all conditioned on the same shared pre-round state (Section~\ref{sec:parallel-decoder}), including each target's remaining value, which suppresses exactly the uninformed pile-on the construction relies on. Second, when collisions do occur, diminishing returns caps the loss of each: the foregone value of a $k$-way collision on target $n$ is at most the sum of the colliding weapons' next-best marginals, not the full value of $k$ engagements. Third, the bound is per-round while the process is multi-round: value left on the table by a collision remains on the battlefield, is re-observed through the updated $\rho_n^{(t+1)}$, and can be recovered in later rounds -- the sequential decoder's informational advantage is strictly within-round. Section~\ref{sec:coordination} shows these mitigations at work empirically.
\end{remark}
...
\textbf{Credit assignment.} We use \emph{reward-to-go}: step $t$'s credit reflects only rewards realized at or after $t$,
\begin{equation}
G_t = \sum_{t'=t}^{T-1} \gamma^{t'-t}\, r_{t'}.
\end{equation}
This return is normalized per instance across the $P$ parallel rollouts (shared baseline, in the manner of~\cite{kwon2020pomo}):
...
%% TODO (NUMBERS PENDING): this section's three tables are the repair layer's evidence and are
%% not yet filled from the current pipeline. Required runs, all already scripted:
%%   (a) per-configuration objective for SCoPE-Comm with and without repair, at matched K, so the
%%       layer's marginal contribution is separable from the construction policy's own quality;
%%   (b) the K-sweep behind Proposition~\ref{prop:repair-monotone} (K = 0, 3, 10, 30, and a
%%       large-budget probe) -- the monotone non-increase is a construction guarantee, but the
%%       SHAPE of the curve, in particular where it flattens, is the empirical claim;
%%   (c) single-flip vs. move-only vs. combined operator under one protocol, supporting the
%%       operator choice asserted in Section~\ref{sec:repair}.
%% Do NOT report the pre-revision numbers from the earlier frozen-base experiments here: those used
%% the one-to-one auction of Section~\ref{sec:auction}'s first paragraph, which Lemma~\ref{lem:dispersion}
%% shows is a different mechanism, and are not comparable to the capacitated pipeline reported above.

The repair layer is evaluated on the same twelve held-out configurations and instance set as Table~\ref{tab:main_results}, applied to the SCoPE-Comm construction policy, with edits selected greedily and the best schedule retained (Section~\ref{sec:repair}).
